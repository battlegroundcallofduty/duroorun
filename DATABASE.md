...
| Gemini API (Google) | AI 리뷰 요약 생성, AI 코스 날씨·안전 브리핑 문구 생성 | 모델: Gemini 3.5 Flash-Lite (두 기능이 같은 클라이언트/모델 공유) |
...
| 한국관광공사 국문 관광정보 서비스 (공공데이터포털) | 코스 시작/종료점 주변 관광지 추천 카드 | 위치기반 관광정보 조회. 캐싱 없이 매 조회마다 실시간 호출(2026-09 결정, 공모전 규정) - 일일 트래픽 한도(1,000회/일)는 IP별 rate limit + 서버 전역 카운터로 보호 (`FEATURES.md` 3-5 참고) |

...
- **해결**: `manual_courses.py`로 하드코딩 데이터+GPX 파일을 보완 데이터로 등록. 이후 DMZ 평화의 길 확장 시 동일 패턴(API 누락)이 재발견되어 같은 방식으로 확장하고, `_merge_manual_courses`가 API 데이터를 나중에 자동으로 우선하도록 병합 로직 작성

...
- **원인**: 처음엔 일시적 API 버그로 의심했으나, 한국관광공사 대외 개방 데이터 운영팀에 문의해 "두루누비 정보 서비스 자체에서 조회 불가능한 데이터"임을 공식 확인(2026-08-24)
- **해결**: `manual_courses.py`로 하드코딩 데이터+GPX 파일을 보완 데이터로 등록(API에는 없지만 두루누비 공식 사이트에서 모든 코스들 정보 확인 가능). 이후 DMZ 평화의 길 확장 시 동일 패턴(API 누락)이 재발견되어 같은 방식으로 확장하고, `_merge_manual_courses`가 API 데이터를 나중에 자동으로 우선하도록 병합 로직 작성
...
- **해결**: 리뷰 도메인에서 이미 검증된 "요청 ID 비교" 가드 패턴을 코드리뷰를 통해 파악하고, 코스/나만의 코스 목록 훅에도 동일한 가드를 적용 — 요청마다 ID를 매겨 응답이 도착했을 때 "지금도 유효한 요청의 응답인지" 확인 후 아니면 버리는 방식으로 재구성
...
- **해결**: 특보 API 가이드와 응답 정보를 다시 살핀 후 특보 조회 방식을 "특보현황조회"로 변경하고, 예보/특보/특보-지역 코멘트 캐시를 분리해 각각 다른 TTL로 재설계. 네트워크 오류 발생 시 날씨 재조회 로직도 추가
